...
describe('DIGIT 16', () => {
  ...
  it('9999999999999999', () => {assert.equal(convert('9999999999999999'), 'девять квадриллионов девятьсот девяносто девять триллионов девятьсот девяносто девять миллиардов девятьсот девяносто девять миллионов девятьсот девяносто девять тысяч девятьсот девяносто девять')});
  it('9999999999999999 num', () => {assert.throws(() => convert(9999999999999999), { name: 'Error', message: 'Нельзя передавать число больше Number.MAX_SAFE_INTEGER. Передайте число строкой, чтобы избежать ошибок округления.' })});
});

describe('DIGIT 18', () => {
  it('1000000000000000000', () => {assert.throws(() => convert(1000000000000000000), { name: 'Error', message: 'Нельзя передавать число больше Number.MAX_SAFE_INTEGER. Передайте число строкой, чтобы избежать ошибок округления.' })});
  it('2000000000000000000', () => {assert.throws(() => convert(2000000000000000000), { name: 'Error', message: 'Нельзя передавать число больше Number.MAX_SAFE_INTEGER. Передайте число строкой, чтобы избежать ошибок округления.' })});
...
describe('Wrong input', () => {
  it('undefined', () => {assert.throws(() => convert(undefined), { name: 'Error', message: 'Нельзя передавать undefined.' })});
  it('null', () => {assert.throws(() => convert(null), { name: 'Error', message: 'Нельзя передавать null.' })});
  it('строка но не число', () => {assert.throws(() => convert('cтрока но не число'), { name: 'Error', message: 'Нужно передать число.' })});
});
